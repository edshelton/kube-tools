...
KUBE_VER=${3-master}
HELM_VER=${4-v2}
HRRENDER="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )/hrrender.sh"

if [[ ${HELM_VER} == "v2" ]]; then
    helm init --client-only
fi

# If the path provided is actually a file, just run HRRENDER against this one file
if test -f "${DIR}"; then
  ${HRRENDER} "${DIR}" "${IGNORE_VALUES}" "${KUBE_VER}" "${HELM_VER}"
  exit 0
fi
...
for f in $(find "${DIR}" -type f -name '*.yaml' -or -name '*.yml'); do
  if [[ $(isHelmRelease ${f}) == "true" ]]; then
    ${HRRENDER} "${f}" "${IGNORE_VALUES}" "${KUBE_VER}" "${HELM_VER}"
